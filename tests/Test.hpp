...
#define UNIT_VERIFY(a, DESC)                                                                              \
    wtr::Testing::verify_impl(a, wtr::Testing::g_test_stack.back().c_str(), __FILE__, __LINE__, \
...
#define CALL_SUBTEST(FUNC)                                                                               \
    do {                                                                                                 \
        MYPRAGMA("omp critical") { wtr::Testing::g_test_stack.emplace_back(PP_MAKE_STRING(FUNC)); } \
        FUNC;                                                                                            \
        MYPRAGMA("omp critical") { wtr::Testing::g_test_stack.pop_back(); }                         \
    } while (0)
