...
MultiSamplePass::MultiSamplePass(int width, int height, int numSample) {
    m_msFBO = std::make_unique<FBO>(width, height, numSample, FBO::Attachments{FBO::Attachments::BUFFER, 1, FBO::Attachments::BUFFER});
    m_txFBO = std::make_unique<FBO>(width, height, 1, FBO::Attachments{FBO::Attachments::TEXTURE, 1, FBO::Attachments::TEXTURE});
}
