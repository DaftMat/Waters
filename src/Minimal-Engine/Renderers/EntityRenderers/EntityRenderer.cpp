...
void EntityRenderer::prepare() const { m_shaderProgram->use(); }
...
void EntityRenderer::setLights(const LightCollection &lights) const {
    ...
    for (const auto &light : lights.pointLights()) m_shaderProgram->addLight(light);
    for (const auto &light : lights.directLights()) m_shaderProgram->addLight(light);
}
